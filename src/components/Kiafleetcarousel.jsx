...
// ─── slot layout config ────────────────────────────────────────────────
// x      = horizontal offset from center (as % of the card's own width)
// scale  = how large the card appears
// y      = vertical nudge in px — positive = push down, so side cards sit
//          lower and feel naturally "behind" the hero stage floor
// opacity/zIndex as before
const DESKTOP_SLOTS = {
...
const MOBILE_SLOTS = {
  "-2": { x: "-230%", scale: 0.42, y:  0,  opacity: 0,    zIndex: 0  },
  "-1": { x: "-100%", scale: 0.58, y: 18,  opacity: 0.55, zIndex: 10 },
  "0":  { x: "0%",    scale: 1,    y:  0,  opacity: 1,    zIndex: 20 },
  "1":  { x: "100%",  scale: 0.58, y: 18,  opacity: 0.55, zIndex: 10 },
  "2":  { x: "230%",  scale: 0.42, y:  0,  opacity: 0,    zIndex: 0  },
};
  ...
  return (
    <div className="w-full bg-white overflow-x-hidden">
      ...
      >
        ...
        >
          <h2 className="text-white text-center tracking-widest sm:text-4xl text-lg"
          ...
          <div className="flex items-center flex-wrap justify-center gap-2 md:gap-6 mt-2">
            ...
            <a href="/vehicles" className="text-white inline-flex items-center gap-1 no-underline text-xs sm:text-lg"
        ...
        {/*
          Key changes here:
          1. The stage now extends beyond the hero width using negative margins
             so side cars have room to breathe before hitting the clip boundary.
          2. overflow is removed from the inner wrapper — the hero's own
             overflow:hidden (on the bg layer) handles visual clipping naturally.
          3. Car images are slightly wider on desktop (600px vs 560px) to fill
             the extra space without looking too small.
        */}
        <div
          className="absolute"
          style={{
            bottom: isMobile ? "-70px" : "-90px",
            // Extend stage left/right beyond the hero container
        ...
        >
          {/* No overflow:hidden here — lets neighbor cards show fully */}
                ...
                >
                  <img
                    ...
                    style={{
                      ...
                      // Slightly wider so cars fill the stage better
          ...
          {/* Prev arrow */}
          <ArrowBtn direction="prev" onClick={prev} isMobile={isMobile} />

          {/* Next arrow */}
          <ArrowBtn direction="next" onClick={next} isMobile={isMobile} />
        </div>
      </div>
...
function ArrowBtn({ direction, onClick, isMobile }) {
  ...
  return (
    <button
      ...
      style={{
        position: "absolute",
        [isPrev ? "left" : "right"]: isMobile ? "12px" : "clamp(8px, 4vw, 56px)",
